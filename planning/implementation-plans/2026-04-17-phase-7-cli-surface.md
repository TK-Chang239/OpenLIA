...
> - `openlia wizard reset` must write the Plan 10 `wizard_state` shape: `current_step="mode"`, `completed_steps=[]`, `active_session_token=None`. The old `current_step=1` integer form is superseded; any snippet in this plan that uses `current_step=1` is pre-audit drift — follow Plan 10 Task 1 instead.
> - Every backend import must match the README "Current backend contract" block in `planning/implementation-plans/README.md` (model locations, `services/auth/passwords`, `services/auth/tokens`, etc.). Where this plan's dependency list above pre-dates that audit, the README wins.
