...
**2026-04-24 amendment.** The shipped Phase-1b schema is 12 tables, not 11. The v2 `rs_classification_log` audit table was appended by migration `2026-04-24-0100_rs_classification_log.py` and lives under `models/dashboard.py`, so it is scoped to Phase 1b for ownership even though the column list is defined in `retail-sentiment-dashboard-design.md`. Migration `2026-04-24-0001_mr_dashboard_state_schedule_cols.py` also added two columns to `mr_dashboard_state` (`assessment_schedule`, `last_assessment_at`) as a follow-up to the background-task scheduler design; those columns are Phase 1b's responsibility. Table totals after the amendment: Plan 1A = 22, Plan 1B = 12, total 1A+1B = 34 (later phases layer in further tables; see `database-design.md` §9 for the full 40-table inventory).
