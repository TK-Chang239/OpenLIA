/**
 * V3ModelPicker — single-slot model picker pill for the v3 engine.
 *
 ...
 * under ``er.v3.model_id`` instead of writing to ``user_prefs``. The
 * v3 engine takes ``provider_kind + model_ref`` directly on its start
...
const LS_KEY = "er.v3.model_id";

export interface V3ModelSelection {
...
interface Props {
  onChange: (selection: V3ModelSelection | null) => void;
}

function entryToSelection(entry: RosterEntry): V3ModelSelection {
  return {
    id: entry.id,
...
export function V3ModelPicker({ onChange }: Props): JSX.Element | null {
  ...
  if (loadError || enabledFlat.length === 0) {
    return (
      <a
        ...
        data-testid="er-v3-model-picker-empty"
  ...
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <button
          type="button"
          aria-label="Choose model for v3 engine"
          data-testid="er-v3-model-picker-trigger"
            ...
            <DropdownMenu.Group key={group.kind}>
              ...
              {group.items.map((m) => {
                ...
                return (
                  <DropdownMenu.Item
                    key={m.id}
                    data-testid={`er-v3-model-option-${m.id}`}
