/**
 * V2EngineModelsPicker — composer-toolbar trigger + modal that lets the user
 * assign a model to each LLM-using stage of the v2.2 equity-research pipeline.
 *
 * The trigger button shows current completion state ("Engine models · 5/8")
 * and turns warning-toned when any slot is unassigned. The modal lists every
 * V2_SLOT with a per-slot dropdown sourced from `/api/settings/models`.
 *
 * State management:
 * - `assignments` is the working draft inside the modal.
 * - `saved` is the last server-acknowledged snapshot.
 * - Save button is disabled when draft === saved (no diff).
 * - Cancel resets draft → saved.
 *
 * The parent reads completion state via the `onAssignmentsChange` callback so
 * it can gate the send button. Initial fetch happens on mount.
 */
...
import {
  ...
  V2_SLOT_DESCRIPTION,
  V2_SLOT_LABEL,
  V2_SLOT_RECOMMENDATION,
  type V2Slot,
  getModelAssignments,
  putModelAssignments,
} from "../../api/er-v2-models";
import { type RosterEntry, getEnabledModels } from "../../api/settings";
import { V2EnginePipelineDiagram } from "./V2EnginePipelineDiagram";
...
export function V2EngineModelsPicker({
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getModelAssignments().catch(() => null),
    ]).then(([m, a]) => {
      if (cancelled) return;
      setModels(m ?? []);
      if (a) propagate(a);
  ...
  const total = V2_SLOTS.length;
  const assignedCount = V2_SLOTS.filter((s) => draft[s]).length;
  const savedAssignedCount = saved
    ? V2_SLOTS.filter((s) => saved.assignments[s]).length
    : 0;
  ...
  const onSelectModel = (slot: V2Slot, modelId: string) => {
    setDraft((prev) => {
      const next = { ...prev };
      if (!modelId) {
        delete next[slot];
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      const next = await putModelAssignments(draft);
      propagate(next);
      setOpen(false);
    } catch (e) {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <button
          type="button"
          aria-label="Configure per-stage engine models"
          data-testid="er-v2-engine-models-trigger"
          className={[
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content
          data-testid="er-v2-engine-models-modal"
        ...
        >
          <div className="flex items-center gap-[10px] border-b border-[--color-border-subtle] px-[22px] py-[16px]">
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              ...
              <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-[--color-text-tertiary]">
                {assignedCount}/{total} slots assigned · v2.2 pipeline
              </span>
            </div>
          ...
          {loadError ? (
            ...
            <div className="flex-1 overflow-auto px-[22px] py-[14px]">
              <details
                open
                className="mb-[14px] rounded-[9px] border border-[--color-border-subtle] bg-[--color-bg-elevated]"
              >
                <summary className="cursor-pointer select-none px-[14px] py-[10px] font-mono text-[10px] uppercase tracking-[0.1em] text-[--color-text-tertiary] hover:text-[--color-text-primary]">
                  Pipeline overview · click to collapse
                </summary>
                <div className="px-[14px] pb-[14px]">
                  <V2EnginePipelineDiagram />
                </div>
              </details>
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-[6px] flex items-start gap-[6px] text-[11px] leading-[1.5] text-[--color-text-tertiary]">
                        ...
                        <span>{V2_SLOT_RECOMMENDATION[slot]}</span>
                      </p>
                    </li>
                  );
                })}
          ...
          <div className="flex items-center justify-between gap-[10px] border-t border-[--color-border-subtle] bg-[--color-bg-base] px-[22px] py-[12px]">
            <span className="font-mono text-[10px] uppercase tracking-[0.1em] text-[--color-text-tertiary]">
              {allAssigned
                ? "ready to run"
                : `${total - assignedCount} slot(s) still unset`}
            </span>
