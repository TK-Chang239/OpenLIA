...
_SUMMARY_WORKFLOW = """\
Work in this order:
  1. Read the cached per-dashboard states in the "# Provided inputs for this
     run" block: the Debt Cycle phase, the Four Seasons season, the World
     Order stage, the All-Weather coverage, and the Five Forces active-count
     and severities. Treat each as the authoritative current read for that
     framework; do not contradict it. Honor any framework the block marks as
     not yet generated by saying so rather than inventing a state.
  2. Gather the live macro signals that frame the synthesis, each with a value
     and an as-of date: a growth nowcast, core PCE, a financial-conditions
     index, the 10-year Treasury yield, key cross-asset quotes, central-bank
     policy rates, the yield-curve slope, and the economic calendar. Prefer
     the enabled connector tools first; fall back to `web_search` of official
     sources.
  3. Synthesize the SummaryData payload: the hero read, the LIA take (exactly
     four pulls), the regime bar, ONE frameworkStatus card per framework
     dashboard (tcode T1-T5, slug, title, stamp status, summary, miniVisual,
     miniData, stats, footLabel) reflecting that dashboard's cached state, the
     dependency map (nodes plus edges between the frameworks) and the cascade
     narrative, and a consolidated watchlist drawn from the individual
     dashboards.
  4. Call `emit_dashboard` exactly once with the full SummaryData object in
     `payload`. This finalizes the run. There is no classifier for the Summary
     view; the synthesis is yours to author from the cached states and the
     live signals."""


_SUMMARY_PAYLOAD_SHAPE = """\
# SummaryData payload shape

`emit_dashboard`'s `payload` is one JSON object. All `status` values are one
of ok/warn/bad/flat/info/acid; `tcode` is one of T1/T2/T3/T4/T5; `miniVisual`
is one of bars/quadrant/ring/stage/forces. Keys:
  - `hero`: {eyebrow, eyebrowStrong (optional), headline, headlineAccent, lede,
    stats: [{k, v, status}]}.
  - `liaTake`: {label, timestamp, paragraphs: [string], pulls: [{k, v}, {k, v},
    {k, v}, {k, v}]} — `pulls` is exactly a 4-element array.
  - `regimeBar`: {label, subLabel, segments: [{k, v, status, sub}]}.
  - `frameworkStatus`: {label, subLabel, cards: [{tcode, slug, acid (optional
    bool), title, stamp: {label, status}, verdictLine (optional), summary,
    miniVisual, miniData, stats: [{k, v, status (optional)}], footLabel,
    spotlight (optional bool), spotlightChart (optional)}]} — `miniData` is
    either a number array (histogram) or {active: bool, index (optional int)};
    `spotlightChart` is {yLabel, yUnit (optional), yMin (optional), yMax
    (optional), data: [{year, value}], current: {year, value}}. Emit one card
    per framework dashboard (T1-T5).
  - `depMap`: {label, subLabel, sub, nodes: [{id, tcode, name, status,
    statusLabel, position}], edges: [{from, to, label, variant}]} — node
    `position` is one of left-top/left-mid/left-bot/center/right; edge uses the
    key `from` (and `to`), and `variant` is one of solid/dashed/accent.
  - `cascade`: {label, subLabel, sub, row1: [{badge, title, body, target
    (optional bool)}], row2: [{badge, title, body, target (optional bool)}]}.
  - `watchlist`: {label, subLabel, triggers: [{status, name, source, desc,
    fromTabs}]}.
  - `sources`: a short string naming the sources you used.
  - `generated_at`: an ISO-8601 timestamp for the run."""
...
DASHBOARD_PROMPT_SPECS: dict[str, DashboardPromptSpec] = {
    ...
    "summary": DashboardPromptSpec(
        workflow=_SUMMARY_WORKFLOW,
        payload_shape=_SUMMARY_PAYLOAD_SHAPE,
        indicator_hint=(
            "a growth nowcast, core PCE, a financial-conditions index, the 10-year Treasury "
            "yield, key cross-asset quotes, central-bank policy rates, the yield-curve slope, "
