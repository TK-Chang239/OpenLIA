...
async def test_disable_native_env_flag_routes_to_configured(monkeypatch) -> None:
    """G-Emergency: OPENLIA_DISABLE_NATIVE_WEB_SEARCH forces native off."""
    monkeypatch.setenv("OPENLIA_DISABLE_NATIVE_WEB_SEARCH", "true")
    adapter = _FakeAdapter(name="brave")
    resolution = resolve_web_search(
        resolved=_resolved(web_search_native=True),
        search_adapter_factory=lambda: adapter,
    )
...
async def test_disable_native_env_flag_routes_to_unavailable_without_fallback(monkeypatch) -> None:
...
async def test_native_web_search_flag_false_routes_to_configured(monkeypatch) -> None:
    """Migration flag: OPENLIA_NATIVE_WEB_SEARCH=false disables native."""
    monkeypatch.setenv("OPENLIA_NATIVE_WEB_SEARCH", "false")
    adapter = _FakeAdapter(name="tavily")
    resolution = resolve_web_search(
        resolved=_resolved(web_search_native=True),
        search_adapter_factory=lambda: adapter,
...
async def test_native_web_search_flag_default_enabled(monkeypatch) -> None:
    monkeypatch.delenv("OPENLIA_NATIVE_WEB_SEARCH", raising=False)
    monkeypatch.delenv("OPENLIA_DISABLE_NATIVE_WEB_SEARCH", raising=False)
    resolution = resolve_web_search(
        resolved=_resolved(web_search_native=True),
        search_adapter_factory=lambda: None,
    )
