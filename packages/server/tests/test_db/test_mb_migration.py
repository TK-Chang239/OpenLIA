...
# the whole MB v2 stack. ``mb_user_configs`` predates MB v2 and is dropped at
# head by the Phase 6 drop migration; downgrading past it here recreates it.
...
def test_migration_upgrade_creates_mb_tables_and_seeds_default(tmp_path, monkeypatch):
    ...
    # The old per-user config table is dropped at head (Phase 6 drop migration,
    # atomically with the removal of its MbUserConfig model) so metadata and the
    # alembic head stay in sync.
...
def test_migration_downgrade_reverses(tmp_path, monkeypatch):
    ...
    # MB v2 tables dropped; mb_user_configs is recreated by the Phase 6 drop
    # migration's downgrade as we step back past head to this revision.
