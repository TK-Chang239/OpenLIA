# packages/server/tests/test_services/test_mb_v2_data_sources.py
...
def test_removing_connector_drops_source(monkeypatch, db_session):
    ...
    db_session.commit()
    ds2 = mb_v2_data_sources.compute_data_sources(
        db_session,
        provider_kind="anthropic",
        model="claude-sonnet-4-6",
        enabled_provider_ids=[],
...
def test_web_search_follows_model_capability(monkeypatch, db_session):
    ...
    yes = mb_v2_data_sources.compute_data_sources(
        db_session,
        provider_kind="anthropic",
        model="claude-sonnet-4-6",
        enabled_provider_ids=[],
    ...
    assert ws_yes.enabled is True

    no = mb_v2_data_sources.compute_data_sources(
        db_session,
        provider_kind="anthropic",
...
def test_web_search_enabled_flag_honored(monkeypatch, db_session):
    monkeypatch.delenv("EODHD_API_KEY", raising=False)
    _add_user(db_session)
    db_session.commit()
    ds = mb_v2_data_sources.compute_data_sources(
...
def test_compute_data_sources_honors_capability_override(db_session):
    ...
    def _ds():
        return mb_v2_data_sources.compute_data_sources(
            db_session,
            provider_kind="anthropic",
    ...
    assert _by_key(_ds())["model_web_search"].available is False
    ...
    assert _by_key(_ds())["model_web_search"].available is True
