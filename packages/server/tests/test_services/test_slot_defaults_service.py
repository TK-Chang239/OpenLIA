...
def test_set_overwrites_existing(db_session, llm_model_factory):
    ...
    set_slot_default(
        db_session, slot_kind="department", slot_id="secretary", model_id=m1.id
    )
    set_slot_default(
        db_session, slot_kind="department", slot_id="secretary", model_id=m2.id
    )
...
def test_invalid_slot_kind_raises(db_session, llm_model_factory):
    ...
    with pytest.raises(InvalidSlotError):
        set_slot_default(
            db_session, slot_kind="bogus", slot_id="x", model_id=model.id
        )
...
def test_list_returns_all_defaults(db_session, llm_model_factory):
    ...
    set_slot_default(
        db_session, slot_kind="system_role", slot_id="ai_review", model_id=m2.id
    )
