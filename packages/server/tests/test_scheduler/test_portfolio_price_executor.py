...
import openlia_server.db.models.register_all  # noqa: F401, E402
...
def _seed_holding(db_session, ticker: str) -> None:
    ...
    db_session.add(
        PortfolioHolding(id=f"h-{ticker}", user_id="u-port-exec", ticker=ticker)
    )
    db_session.commit()


@pytest.mark.asyncio
async def test_executor_runs_refresh_and_writes_quotes(
...
@pytest.mark.asyncio
async def test_executor_records_job_run(
    db_session, session_factory
) -> None:
