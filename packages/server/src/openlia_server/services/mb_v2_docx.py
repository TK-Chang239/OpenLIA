"""DOCX renderer for v2 Morning Briefing reports.

Forked from ``eu_v2_docx.py``. Consumes the same persisted rows the HTML
/ PDF renderers do (``ReportMb`` + sections + charts + citations) and
returns ``.docx`` bytes. Charts try the existing
``report_mb.rendering.chart_renderer`` (matplotlib PNG) and embed the
image; when rendering is impossible (no matplotlib in the env, mis-shaped
data, unsupported chart type) the renderer falls back to a placeholder so
...
``display_index``; the bracketed numbers match the HTML renderer's
output.
...
from openlia.llm.runtime.report_mb import ChartSpec, CoverSpec
from openlia.llm.runtime.report_mb.rendering import strip_anthropic_citation_markup
...
def render_docx(
    *,
    report: ReportMb,
    sections: list[ReportMbSection],
    charts: list[ReportMbChart],
    citations: list[ReportMbCitation],
) -> bytes:
    """Build a Word document from the persisted MB v2 rows."""
    doc = Document()
...
def _configure_styles(doc: Document) -> None:
    ...
    styles; no per-heading overrides needed."""
...
def _parse_cover(raw: str | None) -> CoverSpec | None:
    """Deserialise ``ReportMb.cover_json`` to a ``CoverSpec``. Returns
    None for runs where the model never called ``set_cover`` or when the
    stored JSON can't be parsed — the cover degrades to the bare title +
    eyebrow path."""
    if not raw:
        return None
    try:
        ...
        log.warning("MB v2 cover_json failed to deserialise; rendering bare cover")
        return None


def _add_cover(doc: Document, report: ReportMb, cover: CoverSpec | None) -> None:
    """Emit the cover hero block.

    Bare path (cover is None): centered title + eyebrow line, then a
    spacer. Populated path: adds an optional subtitle above the title, a
    tagline beneath the eyebrow, a rating + upside line, a Highlights
    bullet list, and a key metrics table. Empty fields on a populated
    CoverSpec just skip their row rather than render blank elements.
    """
...
def _format_rating_line(cover: CoverSpec) -> str:
    """Compose a one-line summary, e.g. ``"Risk-on  ·  +1.2% upside"`` or
    just ``"Neutral"`` when no upside. Returns empty when neither field
    is set."""
    parts: list[str] = []
    if cover.rating:
...
def _format_eyebrow(report: ReportMb) -> str:
...
def _add_section(
    *,
    doc: Document,
    section: ReportMbSection,
    md: MarkdownIt,
    display_index_by_source_id: dict[str, int],
    charts_by_id: dict[str, ReportMbChart],
) -> None:
...
def _emit_tokens(doc: Document, tokens: list[Any]) -> None:
    ...
    Implements a focused subset of CommonMark sufficient for section
    bodies: headings, paragraphs, bold / italic / code-inline / links,
    ordered + bullet lists, blockquotes, soft breaks. Tables are passed
    through best-effort (markdown-it table plugin emits th/td tokens we
    map to a docx ``add_table``).

    Unknown token types are silently skipped — better to drop an exotic
    block than crash the whole export.
    """
    i = 0
    while i < len(tokens):
        tok = tokens[i]
...
def _embed_chart(doc: Document, chart: ReportMbChart) -> None:
...
def _try_render_chart_png(chart: ReportMbChart) -> bytes | None:
    """Round-trip the persisted spec_json through ``ChartSpec`` and
    the MB chart renderer. Returns None when matplotlib is absent or
    ...
    try:
        ...
        return None
    try:
        from openlia.llm.runtime.report_mb.rendering.chart_renderer import (
            render_chart_png,
        )
...
def _add_bibliography(doc: Document, citations: list[ReportMbCitation]) -> None:
    ...
    misfires (native search emits non-source-id markers we can't link
    back automatically)."""
    if not citations:
        return
    indexed: list[tuple[int, ReportMbCitation]] = []
    unindexed: list[ReportMbCitation] = []
    max_index = 0
    for c in citations:
