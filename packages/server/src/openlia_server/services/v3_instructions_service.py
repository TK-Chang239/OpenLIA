"""v3 instruction-profile service — resolver + upload + listing + delete.
...
Mirrors ``v3_template_service`` (same owner-scoping, soft-delete, and
listing semantics) so the two artifact types behave identically from
...
from openlia_server.db.models.report_v3 import ReportV3Instructions
...
def resolve_instructions(
    *,
    ...
    """
    row = db.get(ReportV3Instructions, instructions_id)
    if row is None or row.deleted_at is not None:
        raise InstructionsNotFoundError(f"v3 instructions {instructions_id!r} not found")
...
def list_instructions(*, db: DBSession, user_id: str) -> list[InstructionsSummary]:
    ...
    stmt = select(ReportV3Instructions).where(ReportV3Instructions.deleted_at.is_(None))
    ...
    *,
    ...
    if row.is_builtin:
        raise InstructionsNotFoundError(
            f"v3 instructions {instructions_id!r} is a built-in and cannot be deleted"
        )
