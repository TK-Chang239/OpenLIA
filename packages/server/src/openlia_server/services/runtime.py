...
def select_report_runner_class(
    *, department_id: str
    ...
    """Route a department to its report runner class.

    ``OPENLIA_USE_SUBAGENT_RUNNER=1`` + ``equity_research`` selects the
    ``SubagentReportRunner``; all departments otherwise use the classic
    ``ReportRunner``.
    """
