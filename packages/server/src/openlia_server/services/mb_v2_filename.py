"""Shared filename convention for v2 Morning Briefing reports.

Forked from ``eu_v2_filename.py``. Used by:
...
The MB subject is the briefing label (e.g. ``Morning Briefing -
2026-06-02``), not a ticker, so no earnings-specific anchor leaks into
the filename.
"""
...
def slugify_filename_component(value: str) -> str:
    ...
    dashes. Runs of two or more separators collapse to a single
    underscore so a stray "Q&A / Research" becomes "Q-A_Research", not
    "Q-A_-_Research". Lone single separators are preserved so
    "Morning-Briefing" stays intact through the slugifier.
    """
    cleaned: list[str] = []
...
def build_download_filename(*, row: ReportMb, ext: str) -> str:
