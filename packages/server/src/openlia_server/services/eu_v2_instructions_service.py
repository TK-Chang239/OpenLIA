"""EU instruction-profile service — resolver + upload + listing + delete.
...
Forked from ``v3_instructions_service`` (same owner-scoping, soft-delete,
...
from openlia_server.db.models.report_eu import ReportEuInstructions
...
def resolve_instructions(
    *,
    ...
    """
    row = db.get(ReportEuInstructions, instructions_id)
    if row is None or row.deleted_at is not None:
        raise InstructionsNotFoundError(f"eu instructions {instructions_id!r} not found")
...
def list_instructions(*, db: DBSession, user_id: str) -> list[InstructionsSummary]:
    ...
    stmt = select(ReportEuInstructions).where(ReportEuInstructions.deleted_at.is_(None))
    ...
    *,
    ...
    if row.is_builtin:
        raise InstructionsNotFoundError(
            f"eu instructions {instructions_id!r} is a built-in and cannot be deleted"
        )
