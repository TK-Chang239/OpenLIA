"""MB v2 template service — resolver + upload + listing + delete.

The single source of truth for MB v2 templates is the
``report_mb_templates`` table. The runtime resolves a ``template_id``
to a ``TemplateSpec`` through ``resolve_template``; the upload path
parses + compiles + persists via ``create_template_from_markdown``.

The built-in ``mb_default`` row is pre-seeded by the migration and read
through the same row-lookup path as user uploads, so the engine has one
template-loading code path.

Forked from ``eu_v2_template_service.py`` with ``ReportEuTemplate``
swapped for ``ReportMbTemplate`` and the built-in id ``eu_default``
swapped for ``mb_default``. The markdown parser and TemplateSpec
compiler are department-agnostic and shared unchanged.
"""
...
from openlia.llm.runtime.report_mb import TemplateSpec
...
def resolve_template(
    ...
    *,
    ...
    """
    row = db.get(ReportMbTemplate, template_id)
    if row is None or row.deleted_at is not None:
        raise TemplateNotFoundError(f"mb template {template_id!r} not found")
...
def list_templates(db: DBSession, *, user_id: str) -> list[TemplateSummary]:
    ...
    stmt = select(ReportMbTemplate).where(ReportMbTemplate.deleted_at.is_(None))
...
def _compile_mb_template_spec(
    parsed: ParsedTemplate,
    *,
    template_id: str,
    default_name: str,
) -> TemplateSpec:
    """Turn a ``ParsedTemplate`` into an MB v2 ``TemplateSpec``.

    Reads the same frontmatter keys as the EU/v3 compiler — the three are
    structurally identical; only the table they persist into differs. MB
    templates are never ticker-anchored.
    """
    doc_fm = parsed.document_frontmatter or {}
    name = str(doc_fm.get("name") or default_name).strip() or default_name
    shape_description = str(
        doc_fm.get("shape_description") or f"{name} — user-uploaded morning briefing template."
    ).strip()
    ticker_anchored = bool(doc_fm.get("ticker_anchored", False))
...
def create_template_from_markdown(
    *,
    ...
    try:
        spec = _compile_mb_template_spec(
            parsed,
            template_id=template_id,
            default_name=name,
        )
    ...
    *,
    ...
    if row.is_builtin:
        raise TemplateNotFoundError(
            f"mb template {template_id!r} is a built-in and cannot be deleted"
        )
