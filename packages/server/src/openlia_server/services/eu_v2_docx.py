"""DOCX renderer for v2 Earnings Update reports.

Consumes the same persisted rows the HTML / PDF renderers do
(``ReportEu`` + sections + charts + citations) and returns
``.docx`` bytes. Charts try the existing
``report_eu.rendering.chart_renderer`` (matplotlib PNG) and embed
...
from openlia.llm.runtime.report_eu import ChartSpec, CoverSpec
from openlia.llm.runtime.report_eu.rendering import strip_anthropic_citation_markup
...
def _parse_cover(raw: str | None) -> CoverSpec | None:
    """Deserialise ``ReportEu.cover_json`` to a ``CoverSpec``. Returns
...
def _add_cover(doc: Document, report: ReportEu, cover: CoverSpec | None) -> None:
...
def _format_eyebrow(report: ReportEu) -> str:
...
def _add_section(
    *,
    doc: Document,
    section: ReportEuSection,
    md: MarkdownIt,
    display_index_by_source_id: dict[str, int],
    charts_by_id: dict[str, ReportEuChart],
) -> None:
...
def _embed_chart(doc: Document, chart: ReportEuChart) -> None:
...
def _try_render_chart_png(chart: ReportEuChart) -> bytes | None:
    ...
    try:
        ...
        return None
    try:
        from openlia.llm.runtime.report_eu.rendering.chart_renderer import (
            render_chart_png,
        )
...
def _add_bibliography(doc: Document, citations: list[ReportEuCitation]) -> None:
    ...
    if not citations:
        return
    indexed: list[tuple[int, ReportEuCitation]] = []
    unindexed: list[ReportEuCitation] = []
    max_index = 0
    for c in citations:
