...
def auto_map(
    ...
    *,
    ...
    # Track which requirement_types have been seen so we can detect unmet ones.
    seen_req_types: set[str] = set()

    for dep in manifest.departments:
        for req in dep.requirements:
            capable: list[DataProvider] = []
            for prov in providers:
                ...
                if req.type in adapter_cls.capabilities:
                    capable.append(prov)
            if not capable:
                if req.type not in seen_req_types:
                    unmet.append(
