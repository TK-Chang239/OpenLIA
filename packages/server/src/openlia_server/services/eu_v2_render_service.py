"""Server-side wrapper around the EU v2 rendering pipeline.
...
``report_eu_charts.rendered_url`` column so subsequent renders are
...
from openlia.llm.runtime.report_eu.rendering import (
    AssembledReport,
    assemble_html,
)
...
def render_docx(
    *,
    ...
    """Render a persisted EU v2 run to .docx bytes.
    ...
    ``report_eu.rendering.chart_renderer`` and embed a PNG; when
